...
##Filtering for Lipid metabolism

TC1<-subset(TC, rhs=='D052439')
CTDTC1<-subset(CTDTC, rhs=='D052439')
CTD1<-subset(CTD, rhs=='D052439')
...
write.table(TC_perc,file=file.path(outDir, "TC_perc_lm.txt"), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
...
write.table(CTDTC_perc,file=file.path(outDir, "CTDTC_perc_lm.txt"), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
...
write.table(CTD_perc,file=file.path(outDir, "CTD_perc_lm.txt"), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
