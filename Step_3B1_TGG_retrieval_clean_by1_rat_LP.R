...
##Getting LP compounds present in TGG. 
### Adjust accordingly to your local directory
HCC_chem_data <- read.table(paste(Root_location,"/InputFiles/Phospholipidosis_negative_list.txt",sep=''),sep='\t',header=T,comment.char='',strip.white=T)
newcat <- which(caslist2$TGG_compoundName %in% HCC_chem_data$Compound.name)
newcat2 <- caslist2[newcat,]
TGG_in_HCC_list <- unique(merge(caslist2, HCC_chem_data,  by.x="TGG_compoundName", by.y="Compound.name" ,all.x=FALSE, all.y=FALSE))

TGG_in_HCC_list$Casrn <- NULL 
TGG_in_HCC_list<-unique(TGG_in_HCC_list)

TGGinHCC_compoundset <- "/OutputFiles/TGG_in_LP_compounds.txt"
...
HCC_in_TGG_sampleset <- "/OutputFiles/TGG_in_samples_rat_PC.txt"
...
rat_dataset2 <- "/OutputFiles/TGG_data_log2fold_HCC_rat_LP.txt"
